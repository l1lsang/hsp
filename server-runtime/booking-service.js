...
function createSlotRefs(spaceId, startMs, duration) {
...
export async function createBooking(user, body) {
    ...
    if (range.startMs < Date.now())
        throw new HttpError(400, '이미 지난 시간은 예약할 수 없습니다.');
    const { db } = getFirebaseAdmin();
...
export async function cancelBooking(user, bookingId) {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        if (!booking.exists)
            throw new HttpError(404, '예약을 찾을 수 없습니다.');
        const data = booking.data();
        if (data.ownerUid !== user.uid && !user.admin)
            throw new HttpError(403, '취소 권한이 없습니다.');
...
export async function setSpaceStatus(user, spaceId, bookingDisabled) {
    if (!user.admin)
...
export async function createBlock(user, body) {
    if (!user.admin)
        throw new HttpError(403, '관리자 권한이 필요합니다.');
    const range = validateRange(body);
    const { db } = getFirebaseAdmin();
...
export async function listBookings(user, includeAll) {
    const { db } = getFirebaseAdmin();
    ...
    return snapshot.docs.map(document => {
        ...
        const start = koreaDateTime(data.startAt);
        const end = koreaDateTime(data.endAt);
        return {
            id: document.id,
...
export async function getAvailability(spaceId, date) {
    ...
    return {
        slots: slots.docs.map(document => koreaDateTime(document.data().startsAt).time),
